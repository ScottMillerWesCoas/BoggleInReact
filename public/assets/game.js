...
var Box = require('./box'); 
var HighScorers = require('./highScorers'); 
var wordBuild = require('./wordBuild'); 
var scoreBoard = require('./scoreBaord'); 
...
class Game extends React.Component {
  constructor(props){
    ...
    //Lists top scorers at top of game, player's name will be added to list when score 
    //exceeds listed scores
    _highScore(){
    ...
    //Upon submitting a valid word, player's new score is checked against existing top scores
    //to determine if player should be included in top scorers list, if new score is a top score,
    //lower score is removed from list and player's name and score are added and entered into local
    //storage so new top score persists on page reload
    _checkHighScore(){
    ...
    //every time a letter is clicked, it's entered into the wordBuild Array, displayed in 
    //wordBuild component and a only letters adjacent to the letter just clicked can now 
    //be clicked on the game board
    _update(e){
      for (var a = 0; a < 25; a++){
    ...
    //allows unclick functionality, pops last letter off of wordBuild Array, out of wordBuild
    //component and changes clickable letters back to previous arrangement
    _unclick(e){
    ...
    //submit completed word for points, checks against dictionary of valid English words in 
    //dictionary.txt, points added, valid word notification appears, word placed in submitted
    //word component
    _submit(e){ 
      var that = this; 
      ...
      //randomizes letters for board, retrieves highScores from local storage, if any
      _startValue(){
        var winnerList = [
         {name: 'Mike', score: 3},
         {name: 'Scott', score: 0}
        ];
        //Below line to reset highScores to low numbers 
